...
class data_helper:
    ...
    def next_batch(self,size):
        train_x=self.train_x[(self.random_strat+self.batch_num*size)%len(self.train_x):
                             min(self.random_strat+((self.batch_num+1)*size)%len(self.train_x),len(self.train_x))]
        train_y = self.train_y[(self.random_strat+self.batch_num*size)%len(self.train_y):
                               min(self.random_strat+((self.batch_num+1)*size)%len(self.train_y),len(self.train_y))]
        if len(train_x)!=size:
            self.batch_num=0
        else:
            self.batch_num += 1
        self.random_strat=rd.randint(0,size-2)
